...
/*
    Copyright (C) 2014  Mirza Saikat Ahmmed  mirzasaikatahmmed@gmail.com
...
function msnb_notice_shortcode( $atts ){
	...
	if( $query->have_posts() ){
		...
		if('table'==$format){
			...
			$html.= "<tr><th>ক্রমিক নম্বর</th><th>তারিখ</th><th>বিষয়</th><th>বিস্তারিত</th></tr>";
			$sl = ০১;
			while ( $query->have_posts() ) {
				$query->the_post();
				$html.= '<tr><td>'.$sl.'</td><td>'. date_i18n('j F, Y', strtotime(get_the_time('Y-m-d', $post->ID)), false, true, 'bn_BD') .'</td><td>'. get_the_title() .'</td>';
				if ( get_post_meta( $post->ID, 'msnb_notice_url', true ) == '' ) {
					$html.= '<td> <a href="' . 
						get_permalink() . '"> বিস্তারিত </a></td>';
				}
				else {
